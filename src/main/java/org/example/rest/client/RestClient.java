...
import okhttp3.*;
import org.example.rest.client.exception.RestClientException;
...
public abstract class RestClient<T, R> {
    ...
    protected R excecuteAndGetList(Request request) {
        try (Response response = client.newCall(request).execute()) {
            String json = response.body().string();
    ...
    protected RequestBody createBody(T object){
        return RequestBody.create(gson.toJson(object), MediaType.get("JSON"));
    }
}
